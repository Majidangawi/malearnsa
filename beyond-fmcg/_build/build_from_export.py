...
for p in sorted(prods, key=lambda x: str(x["name"]).lower()):
    ...
    catalog.append({"id": pid, "o": (float(p["sort_order"]) if str(p.get("sort_order") or "").strip() not in ("", "None") else None), "n": str(p["name"]).strip(), "n_ar": str(p.get("name_ar") or ""), "c": str(p["code"]), "u": str(p["ean"]), "bn": ", ".join(multi(p["brand"])), "cn": ", ".join(multi(p["category"])), "b": bids, "pl": plids, "k": sorted(set(k)), "i": imgs[0] if imgs else None})
