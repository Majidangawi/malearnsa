...
(async function () {
  const { t, esc, withLang, ICONS } = BF; BF.header({ page: "quote" }); fetch(BF.base + "data/taxonomy.json").then(r => r.json()).then(tax => { BF.applySettings(tax.settings); BF.footer(tax.categories); }).catch(() => BF.footer([]));
